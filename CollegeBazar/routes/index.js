...
router.post('/addItem', isLoggedIn,isBlacklisted,upload.array('image'), async (req, res) => {
    let { item_name, price, category, desc } = req.body;
...
router.post('/tags', isLoggedIn, async (req, res) => {
    ...
    const items = await Item.find({ "title": searchName })
    // console.log(items)
...
router.delete("/deleteCart/:id", isLoggedIn, async (req, res) => {
    const _id = req.params.id
    await User.findByIdAndUpdate(req.user._id, { $pull: { cart: _id } })
    res.redirect("/cart")
})

router.delete("/deleteWishlist/:id", isLoggedIn, async (req, res) => {
    const _id = req.params.id
...
router.post('/addToWishlist/:id', isLoggedIn, async (req, res) => {
    const { id } = req.params
    const item = await Item.findById(id)
    const user = await User.findById(req.user._id)
...
router.put('/allDeets', async (req, res) => {
    const user = req.user;
...
router.put('/edit/:item', isLoggedIn, (req, res) => {
    ...
    // console.log(title)
    Item.findOneAndUpdate({ _id: id }, { "$set": { "title": title, "price": price, "description": description, "image": image} },
        function (err) {
            if (!err) {
                res.redirect("/addItem")
            }
...
router.post('/pillFilter', async (req, res) => {
    ...
    const items = await Item.find({ "category": submit }).populate('author')
    res.render('marketplace.ejs', { items })
